...
(function () {
  ...
  function makeRule(text, sheet, index) {
    ...
    // The rule's CSSStyleDeclaration: reads/writes the declaration block between `{`...`}`.
    var styleObj = makeStyleDecl(
      function () {
        var t = sheet ? sheet.__ruleCssText(index) : text;
        return ruleBlockText(t);
      },
      function (block) {
        if (sheet && sheet.__setRuleBlock) { sheet.__setRuleBlock(index, block); }
      }
    );
    Object.defineProperty(rule, "style", { get: function () { return styleObj; }, enumerable: true, configurable: true });
    return rule;
  }
  // Extract the declaration text inside a rule's `{ ... }` braces (without the braces).
  function ruleBlockText(t) {
    t = String(t);
    var a = t.indexOf("{"); var b = t.lastIndexOf("}");
    if (a < 0) return "";
    return t.slice(a + 1, b >= 0 ? b : t.length).trim();
  }
  function makeStyleSheet(styleEl) {
    ...
    var ss = {
      ...
      // Rewrite the declaration block (inside the braces) of rule `index`.
      __setRuleBlock: function (index, block) {
        var rs = rawRules();
        if (index < 0 || index >= rs.length) { return; }
        var sel = (rs[index].split("{")[0] || "").trim();
        rs[index] = sel + " { " + block + " }";
        styleEl.textContent = rs.join("\n");
      },
      get cssRules() {
